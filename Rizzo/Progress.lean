...
lemma select_cases {b1 b2 : Bool} : b1 ∨ b2 → (b1 ∧ ¬ b2) ∨ (b2 ∧ ¬ b1) ∨ (b1 ∧ b2) := by
  intros B
  cases B <;> grind
...
theorem Adv.progress {v : MVal} : v.ticked ε.now e.chan →  ⊢[ε.chans] e ∷Event → ⊩{ε} v ∷ delayE A → ∃ v' ε', (v, ε) [e]⇘  (v', ε') := by
  intros C I T
  rcases v with ⟨t,p⟩
  simp at *
  revert A ε
  ...
  case wait κ' =>
    ...
    simp[MVal.ticked,Term.ticked] at C
    subst C
    cases T with | chan hT
    simp [IsEvent] at I
    obtain ⟨B, hlook, hw⟩ := I
    have hAB : B = A := by rw [hlook] at hT; simpa using hT
    subst hAB
    obtain ⟨v'', ε'', hev⟩ := Eval.progress ⟨HasType.le hw (AList.le_empty _) (AList.refl _), Tenv⟩
    exact ⟨v''.val, v''.property, ε'', Adv.wait hev⟩
  case never => simp[MVal.ticked,Term.ticked] at C
  case tail l =>
    exact ⟨Term.loc l, IsMValue.loc, ε, Adv.tail⟩
  ...
  case select t1 t2 V1 V2 IH1 IH2 =>
    ...
    . have T1' := EvalType.mk T1 T.env
      ...
      have T2' := EvalType.mk T2 T.env
      have S := (R1.incr)
      have T2'' := T2'.le S T3.env
      have C' := C.2
      rw [T2.ticked_Sub S.store.now] at C'
      have R2 := IH2 (I.le S.chans) C' T2''
      rcases R2 with ⟨ w2, W2, ε'', R2⟩
      exists in2 (pair w1 w2), .in2 (.pair W1 W2), ε''
      apply Adv.select3 C.1 C.2 R1 R2
  case watch l =>
...
theorem Updates.progress : ⊢[ε.chans] e ∷Event → ⊢ ε ∷Env → ∃ ε', ε [e]⇒* ε' /\ ε'.earlier = ∅ := by
  intros I T
...
theorem ReactStep.progress : ⊢[Δ] e ∷Event → ⊢[Δ] η ∷now → ∃ η' Δ' , (v, η, Δ) [e]⟹ (v, η', Δ') := by
  intros I T
